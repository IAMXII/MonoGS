...
class GaussianModel:
    ...
    def generate_points_in_plane_area(self, plane_normal, plane_point, num_points, x_range, y_range):   # liuwei
        """
        Generates points on a plane within a specified rectangular area.

        :param plane_normal: A 3-element array-like representing the normal vector of the plane
        :param plane_point: A 3-element array-like representing a point on the plane
        :param num_points: The number of points to generate
        :param x_range: A tuple (min_x, max_x) defining the range of x coordinates in the plane
        :param y_range: A tuple (min_y, max_y) defining the range of y coordinates in the plane
        :return: A (num_points, 3) array of points on the plane within the specified area
        """
        ...
        # Generate random points within the specified range in 2D
        a = np.random.uniform(x_range[0], x_range[1], num_points)
        b = np.random.uniform(y_range[0], y_range[1], num_points)

        # Map 2D points to 3D plane
        points = plane_point + a[:, np.newaxis] * v1 + b[:, np.newaxis] * v2
        return points
    ...
    def insert_background(self):   # liuwei
        ...
        # if init:
        #     print("dist2_scales", torch.sqrt(dist2[0]))
        #     print("0.0023:", torch.sqrt(torch.tensor(0.0023)))
        # if init:
        #     print("initial scale", scales.cpu().numpy())
        ...
        self.extend_from_pcd(
            fused_point_cloud, features, scales, rots, opacities, 0
        )
    ...
    def create_pcd_from_image_and_depth(self, cam, rgb, depth, init=False):
        ...
        pcd_tmp = o3d.geometry.PointCloud.create_from_rgbd_image(
            ...
            extrinsic=W2C,
            project_valid_depth_only=True,  ### liuwei
        )
        # if init:
        #     o3d.io.write_point_cloud("pcd_tmp_3.pcd", pcd_tmp)
        # pcd_tmp = pcd_tmp.random_down_sample(1.0 / downsample_factor)    ### liuwei
        # pcd_tmp = pcd_tmp.voxel_down_sample(0.1)
        # if init:
