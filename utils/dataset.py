...
class MonocularDataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.has_depth:
            ...
            #             f.write(str(depth[i][j]*self.depth_scale)+'\t')
            #         f.write('\n')
        ...
        # pose = torch.from_numpy(pose).to(device=self.device)   ### liuwei
        pose = torch.eye(4).to(device=self.device)
...
class TUMDataset(MonocularDataset):
    def __init__(self, args, path, config):
        ...
        # self.poses = parser.poses    ### liuwei
        self.color_paths = []
        self.depth_paths = []
        color_path = os.path.join(dataset_path, "images_lidar_2")    # images_lidar_2
        color_paths = sorted(
            os.listdir(color_path), key=lambda x: int("".join(filter(str.isdigit, x)))
        )
        for path in color_paths:
            self.color_paths.append(os.path.join(color_path, path))
        with open("path.txt", "a") as f:
            f.write(str(self.color_paths))

        depth_path = os.path.join(dataset_path, "depths_lidar_2")  # depths_lidar_2
        depth_paths = sorted(
            os.listdir(depth_path), key=lambda x: int("".join(filter(str.isdigit, x)))
        )
        for path in depth_paths:
            self.depth_paths.append(os.path.join(depth_path, path))
