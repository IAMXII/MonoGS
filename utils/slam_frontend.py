...
class FrontEnd(mp.Process):
    ...
    def add_new_keyframe(self, cur_frame_idx, depth=None, opacity=None, init=False):
        ...
        if self.monocular:  ### liuwei
            if depth is None:
                ...
                # )  ### liuwei
                # _, width, height = depth_get.shape
                ...
                # scale_value = scale
                # initial_depth = depth
                # return initial_depth.cpu().numpy()[0], scale_value
            else:
                ...
                if use_inv_depth:
                    ...
                    if len(ratios) < 20:
                        ...
                        # slope,intercept = np.polyfit(depth_render_pixel, depth_gt_pixel, 1)
                        # scale = slope
                        x = np.ones_like(ratios)
                        scale = np.polyfit(ratios, x, 0)
        ...
        scale_value = scale.item() if isinstance(scale, torch.Tensor) else scale
        print("scale_value:", scale_value)
        return initial_depth[0].numpy(), scale_value
    ...
    def initialize(self, cur_frame_idx, viewpoint):
        ...
        depth_map, scale = self.add_new_keyframe(cur_frame_idx, init=True)
    ...
    def tracking(self, cur_frame_idx, viewpoint):
        ...
        # prev_prev = self.cameras[cur_frame_idx - 2 * self.use_every_n_frames]
        # inv_R1 = torch.inverse(prev_prev.R)
        # R = torch.matmul(inv_R1, prev.R)
        # R = torch.matmul(R, prev.R)
        # T = prev.T - prev_prev.T
        # with open("x.txt", "a") as f:
        #     f.write(str(prev.R) + " R\n")
        #     f.write(str(prev.T) + " T\n")
        # prev_prev = self.cameras[cur_frame_idx - 2 * self.use_every_n_frames]
        # inv_R1 = torch.inverse(prev_prev.R)
        # R = torch.matmul(inv_R1, prev.R)
        # R = torch.matmul(R, prev.R)
        # T = 2 * prev.T - prev_prev.T
        # viewpoint.update_RT(R, T)  ### liuwei
            ...
            if self.frontend_queue.empty():
                ...
                if create_kf:
                    ...
                    if self.monocular and not self.initialized and removed is not None:
                        ...
                        continue
                    depth_map, scale = self.add_new_keyframe(
                    ...
                    # if abs(self.cameras[cur_frame_idx].scale-prev_scale)>0.1:
                    #     cur_frame_idx=0
                    #     prev_scale=scale
